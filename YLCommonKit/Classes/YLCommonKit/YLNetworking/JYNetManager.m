...
#import "JYNetManager.h"
#import "JYResponseTip.h"
#import <Reachability.h>
#import <AFNetworkReachabilityManager.h>

/** 网络通知 */
NSString * const JYAPPBecomeActiveNotification     = @"JYAPPBecomeActiveNotification";
NSString * const JYAPPBecomeUnActiveNotification   = @"JYAPPBecomeUnActiveNotification";

static JYNetManager *__shareNetManager = nil;

@interface JYNetManager()
...
- (instancetype)init
{
    if (self = [super init]){
        ...
        //无网络监听
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(yl_cancelAllRequest) name:JYAPPBecomeUnActiveNotification object:nil];
        //创建网络监听实例
...
- (BOOL)adjustQueueItemType:(NSInteger)type flag:(NSInteger)flag
{
    ...
    [self.afRequestManager.tasks enumerateObjectsUsingBlock:^(NSURLSessionTask * _Nonnull task, NSUInteger idx, BOOL * _Nonnull stop) {
        JYResponseTip *tip = [task.userinfo valueForKeyPath:kResponseTip];
...
- (void)dealloc
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:JYAPPBecomeUnActiveNotification object:nil];
}
...
- (void)reachabilityChanged:(NSNotification*)notification
{
    ...
    switch (netStatus){
            case NotReachable:{
                [[NSNotificationCenter defaultCenter] postNotificationName:JYAPPBecomeUnActiveNotification object:nil];
                break;
            }
            case ReachableViaWWAN:{
                [[NSNotificationCenter defaultCenter] postNotificationName:JYAPPBecomeActiveNotification object:nil];
                break;
            }
